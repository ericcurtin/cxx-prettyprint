...
int main(int argc, char* argv[]) {
  ...
  std::cout << "Vector: " << v << std::endl  // vector of strings
            << "Incremental vector: " << vv
            << std::endl                     // nestes vector of vectors
            << "Pairs: " << vp << std::endl  // vector of pairs
            << "Another vector: " << vd
            << std::endl  // vector of doubles using Customization #1
            << "Set: " << ss << std::endl  // set of strings, using partially
                                           // specialized default set delimiters
            << "OMap: " << om
            << std::endl  // associative container (value type is a std::pair)
            << "UMap: " << um << std::endl    // ditto
            << "String: " << cs << std::endl  // just a plain string, note that
                                              // std::string has begin()/end()
            << "Array: " << a << std::endl    // an std::array
      ;

  /* Demo: Here we use our reusable delimiter class MyDelims by directly
   * accessing some interna. */
  std::cout
      << pretty_print::print_container_helper<std::vector<std::string>, char,
                                              std::char_traits<char>, MyDelims>(
             v)
      << std::endl;
  ...
  */
  ...
  std::tr1::tuple<std::string, std::tr1::tuple<std::string, int>, int> a3 =
      std::tr1::make_tuple("Qrgh", a1, 11);
  std::tr1::tuple<int, int, std::pair<double, std::string> > a4 =
      std::tr1::make_tuple(1729, 2875,
                           std::pair<double, std::string>(1.5, "abc"));
